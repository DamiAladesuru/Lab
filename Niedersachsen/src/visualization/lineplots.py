...
# %%
gld_subsets, yearly_desc_dict, griddfs, griddfs_ext, mean_median, gridgdf = describe.process_descriptives()
...
for key, df in mean_median.items():
    ...
    sns.lineplot(data=df, x='year', y='mean_grid_par_diff12', color='purple', label=key)
    # Set the plot title and labels
    plt.title('Trend in Average Change in PercentageSum-AreaSum Ratio of GridCells')
    plt.xlabel('Year')
    plt.ylabel('Mean of Grid PAR Difference')
    # Show the plot
    plt.legend()
    plt.show()
    #save plot
...
for key, df in griddfs_ext.items():
    ...
    sns.scatterplot(data=df, x='mfs_ha', y='grid_par', color='purple', label=key)
    # Add line of best fit
    sns.regplot(data=df, x='mfs_ha', y='grid_par', scatter=False, color='blue')
    # Set the plot title and labels
    plt.title('Correlation between Grid Mean Field Size and Grid PA ratio')
    plt.xlabel('Mean Field Size')
    plt.ylabel('Grid PA ratio')
    # Show the plot
    plt.legend()
    plt.show()
    # Save plot
...
for key, df in griddfs_ext.items():
    ...
    sns.scatterplot(data=df, x='mfs_ha_diff_from_2012', y='mean_cpar_diff_from_2012', color='purple', label=key)
    # Add line of best fit
    sns.regplot(data=df, x='mfs_ha_diff_from_2012', y='mean_cpar_diff_from_2012', scatter=False, color='blue')
    # Set the plot title and labels
    plt.title('Correlation between Change in Grid Mean Field Size and Mean C. Peri-Area ratio')
    plt.xlabel('mfs_ha_diff_from_2012')
    plt.ylabel('mean_cpar_diff_from_2012')
    # Show the plot
    plt.legend()
    plt.show()
    # Save plot
...
for key, df in griddfs_ext.items():
    # Create scatter plot of chnage in grid mfs_ha and grid shp index
    sns.scatterplot(data=df, x='mfs_ha_diff_from_2012', y='grid_par_diff_from_2012', color='purple', label=key)
    # Add line of best fit
    sns.regplot(data=df, x='mfs_ha_diff_from_2012', y='grid_par_diff_from_2012', scatter=False, color='blue')
    # Set the plot title and labels
    plt.title('Correlation between Change in Grid Mean Field Size and Grid PeriSum-AreaSum ratio')
    plt.xlabel('mfs_ha_diff_from_2012')
    plt.ylabel('grid_par_diff_from_2012')
    # Show the plot
    plt.legend()
    plt.show()
    # Save plot
